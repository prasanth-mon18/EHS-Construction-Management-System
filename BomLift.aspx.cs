...
{
    public partial class BomLift : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = CreateBomStructure();

            DataRow dr = dt.NewRow();
            SetDefaultRowFlags(dr);
            dt.Rows.Add(dr);

            ViewState["CurrentBomData"] = dt;
            BindGridData();
        }

        private DataTable CreateBomStructure()
        {
        ...
        {
            DataTable dt = (DataTable)ViewState["CurrentBomData"];
            GridBomlift.DataSource = dt;
            GridBomlift.DataBind();

            ScriptManager.RegisterStartupScript(this, this.GetType(), "initSelect2Bom", "initSearchableDropdowns();", true);
        }
        ...
        protected void btnAddNewRow_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in GridBomlift.Rows)
            {
        ...
        protected void GridBomlift_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void GridBomlift_RowCreated(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void btnSaveBomLift_Click(object sender, EventArgs e)
        {
            ...
            using (OracleConnection conn = new OracleConnection(connString))
            {
                ...
                foreach (GridViewRow row in GridBomlift.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        string insertQuery = @"INSERT INTO VENDOR_BOMLIFT_RESULTS (
        ...
        private void BindHistoryLog()
        {
            using (OracleConnection conn = new OracleConnection(connString))
            {
                ...
                string query = @"SELECT * FROM VENDOR_BOMLIFT_RESULTS" + conditionClause + " ORDER BY ID DESC";
        ...
        protected void GridHistory_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            using (OracleConnection conn = new OracleConnection(connString))
            {
                string sql = "UPDATE VENDOR_BOMLIFT_RESULTS SET DEL_FLAG = 'Y', UPDATED_DATE = SYSDATE WHERE ID = :id";
        ...
        protected void GridHistory_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "RestoreRecord")
            {
                using (OracleConnection conn = new OracleConnection(connString))
                {
                    string sql = "UPDATE VENDOR_BOMLIFT_RESULTS SET DEL_FLAG = 'N', UPDATED_DATE = SYSDATE WHERE ID = :id";
                    using (OracleCommand cmd = new OracleCommand(sql, conn))
                    {
