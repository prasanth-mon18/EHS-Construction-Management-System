...
{
    public partial class SkyLift : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = CreateSkyStructure();

            DataRow dr = dt.NewRow();
            SetDefaultRowFlags(dr);
            dt.Rows.Add(dr);

            ViewState["CurrentSkyData"] = dt;
            BindGridData();
        }

        private DataTable CreateSkyStructure()
        {
            ...
            string[] flags = {
                "HY_CYLINDER_HYD", "HY_FORCES_FUNC", "HY_BODY_STRUCTURE",
                "WD_BUZZER_NOISE", "WD_AMBER_ALARM", "WD_HORN",
                "SF_LOCK_COND", "SF_CTRL_GEARS", "SF_CONNECTION_COND", "SF_BUCKET_COND", "SF_LABEL_TAG",
                "EP_ENGINE_COND", "EP_MAINT_CHECK", "EP_STABILZ_COND", "EP_TIRES_WHEELS", "EP_BRAKE_FUNCT"
        ...
        {
            DataTable dt = (DataTable)ViewState["CurrentSkyData"];
            GridSkylift.DataSource = dt;
            GridSkylift.DataBind();

            ScriptManager.RegisterStartupScript(this, this.GetType(), "initSelect2Sky", "initSearchableDropdowns();", true);
        }
        ...
        protected void btnAddNewRow_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in GridSkylift.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    string[] prefix = {
                        "CylHydr", "ForcesFunc", "BodyStruc",
                        "BuzzerNoise", "AmberAlarm", "Horn",
                        "LockCond", "CtrlGear", "ConnectCond", "BucketCond", "LabelTag",
                        "EngineCond", "MaintCheck", "StablzCond", "WheelCond", "BrakeFunct"
                    };
                    string[] dbFields = {
                        "HY_CYLINDER_HYD", "HY_FORCES_FUNC", "HY_BODY_STRUCTURE",
                        "WD_BUZZER_NOISE", "WD_AMBER_ALARM", "WD_HORN",
        ...
        protected void GridSkylift_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void GridSkylift_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                if (grid.Controls.Count > 0 && grid.Controls[0] is Table)
                {
                    ...
                    TableCell cellMach = new TableCell { Text = "Machinery Details", ColumnSpan = 9, RowSpan = 2, HorizontalAlign = HorizontalAlign.Center, VerticalAlign = VerticalAlign.Middle };
                    ...
                    // Array of your exact checklist parameters descriptions from your paper form image
                    string[] itemDescriptions = {
                        "Cylinder hydraulic", "Forces lift/turn/up", "Body structure",
                        "Buzzer noise", "Amber flashing beacon/ alarm", "Horn",
                        "Lock in good condition", "Control panel/ gears", "Connection condition", "Bucket condition (Clean)", "Information/ Label/ Safe Tagging",
                        "Engine condition", "Maintenance check", "Leg/Stabilisers condition", "Tires/wheel condition", "Brake well functioning"
                    };
                    ...
                    e.Row.Cells[5].Text = "License H";
                    e.Row.Cells[6].Text = "SDIEM PIC";
                    e.Row.Cells[7].Text = "Group PIC";
                    e.Row.Cells[8].Text = "Valid until";

                    for (int k = 9; k < e.Row.Cells.Count; k++)
                    {
        ...
        protected void btnSaveSkyLift_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in GridSkylift.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (!string.IsNullOrEmpty(pma) || !string.IsNullOrEmpty(swl) || !string.IsNullOrEmpty(comp))
                    {
                        ...
                        if (string.IsNullOrEmpty(license)) { lblValidationError.Text = "Error: 'License H' cannot be left empty."; return; }
            ...
            using (OracleConnection conn = new OracleConnection(connString))
            {
                ...
                foreach (GridViewRow row in GridSkylift.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        string insertQuery = @"INSERT INTO VENDOR_SKYLIFT_RESULTS (
                            PMA, SWL, COMPANY_VENDOR, NO_OF_OPERATOR, DRIVING_LICENSE, SDIEM_PIC, GROUP_PIC, VALID_UNTIL,
                            HY_CYLINDER_HYD,HY_FORCES_FUNC,HY_BODY_STRUCTURE, 
                            WD_BUZZER_NOISE,WD_AMBER_ALARM,WD_HORN,
                            SF_LOCK_COND,SF_CTRL_GEARS,SF_CONNECTION_COND,SF_BUCKET_COND,SF_LABEL_TAG,
                            EP_ENGINE_COND,EP_MAINT_CHECK,EP_STABILZ_COND,EP_TIRES_WHEELS,EP_BRAKE_FUNCT
                        ) VALUES (
                            :pma, :swl, :comp, :noOper, :license, :cPic, :gPic, :vDate,
        ...
        private void BindHistoryLog()
        {
            using (OracleConnection conn = new OracleConnection(connString))
            {
                ...
                string query = @"SELECT * FROM VENDOR_SKYLIFT_RESULTS" + conditionClause + " ORDER BY ID DESC";
        ...
        protected void GridHistory_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                TableCell cellMach = new TableCell { Text = "Machinery Details", ColumnSpan = 9, RowSpan = 1, HorizontalAlign = HorizontalAlign.Center, VerticalAlign = VerticalAlign.Middle };
        ...
        protected void GridHistory_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            using (OracleConnection conn = new OracleConnection(connString))
            {
                string sql = "UPDATE VENDOR_SKYLIFT_RESULTS SET DEL_FLAG = 'Y', UPDATED_DATE = SYSDATE WHERE ID = :id";
        ...
        protected void GridHistory_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "RestoreRecord")
            {
                using (OracleConnection conn = new OracleConnection(connString))
                {
                    string sql = "UPDATE VENDOR_SKYLIFT_RESULTS SET DEL_FLAG = 'N', UPDATED_DATE = SYSDATE WHERE ID = :id";
                    using (OracleCommand cmd = new OracleCommand(sql, conn))
                    {
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Sky_Lift_Report_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls");
            Response.Charset = "";
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                // Hide columns
                GridHistory.Columns[27].Visible = false;
                ...
                {
                    for (int i = 0; i <= 25; i++) { GridHistory.HeaderRow.Cells[i].BackColor = System.Drawing.ColorTranslator.FromHtml("#F2F2F2"); }
                }
